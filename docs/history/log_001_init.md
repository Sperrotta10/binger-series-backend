...
- Se construyó el servidor con un `health-check` avanzado y soporte para *Graceful Shutdown*.
...
1. [ ] Arrancar Docker Desktop y ejecutar `pnpm db:migrate` para inicializar y sincronizar PostgreSQL en local.
2. [ ] Validar que `pnpm dev` levante el servidor de forma exitosa y responda al *health-check*.
3. [ ] Iniciar el **Sprint 2: Módulo de Autenticación** comenzando con el endpoint de registro (`/api/v1/auth/register`).
