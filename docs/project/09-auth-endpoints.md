...
**Descripción:** Registro tradicional de usuarios mediante correo y contraseña. El usuario elige explícitamente su nombre de usuario público (`username`).

* **Headers:** `Content-Type: application/json`
* **Body (Payload):**
```json
{
  "fullName": "Santiago Perrotta",
...
* **Reglas de Validación:**
  1. **`fullName`:** Cadena de texto, mínimo 1 carácter, máximo 50.
  2. **`username`:** Alfanumérico en minúsculas y guiones bajos (`/^[a-z0-9_]+$/`), 3-30 caracteres, debe ser único.
  3. **`email`:** Formato de email válido, debe ser único.
  4. **`password`:** Mínimo 8 caracteres, al menos una letra y un número.
...
* **`400 Bad Request`:** Contraseña débil, formato de email inválido o username con caracteres no permitidos.
* **`409 Conflict`:** El correo electrónico (`ACCOUNT_EXISTS`) o el username (`USERNAME_TAKEN`) ya se encuentran registrados.
...
```json
{
  ...
  "data": {
    "user": {
      ...
      "username": "santiago_dev",
      "fullName": "Santiago Perrotta",
      "avatar_url": "https://api.binger.com/assets/avatars/default.webp"
    },
    "tokens": {
      "accessToken": "eyJhbGciOi...",
      "refreshToken": "def456..."
...
* **Respuestas:**
* **`200 OK` / `201 Created`:** El backend valida el token con Google. Si el usuario no existía, lo crea automáticamente (*idempotencia*) y genera un nombre de usuario base desde el email. Devuelve la misma estructura de tokens que el login convencional.
...
**Descripción:** Obtiene la información completa del perfil del usuario autenticado. Se utiliza al cargar la pantalla de "Editar Perfil" para precargar los campos de texto.
    ...
    ```json
    {
      ...
      "data": {
        "user": {
          ...
          "avatar_url": "https://api.binger.com/assets/avatars/santiago.webp",
...
**Descripción:** Modifica los datos del perfil del usuario (nombre completo, nombre de usuario, biografía y avatar). Todos los campos en el body son opcionales para permitir actualizaciones parciales.

* **Headers:** `Authorization: Bearer <accessToken>`
* **Body (Payload):**
```json
{
  "username": "santiago_cinephile",
  "fullName": "Santiago Perrotta",
  "biography": "Backend Developer & Movie Critic. Currently tracking Succession.",
  "avatar_url": "https://api.binger.com/assets/avatars/new_avatar.webp"
}

```

* **Reglas de Validación en Backend:**
1. **Formato del `username`:** Debe ser estrictamente alfanumérico, permitir guiones bajos (`_`), minúsculas y no contener espacios (ej: regex `/^[a-z0-9_]+$/`). Mínimo 3, máximo 30 caracteres.
2. **Límite del `fullName`:** Cadena de texto, mínimo 1 carácter, máximo 50 caracteres.
3. **Límite de la `biography`:** Texto plano truncado a un máximo de 160 caracteres (estilo Twitter/Letterboxd).
...
* **`409 Conflict`:** El `username` solicitado ya está siendo utilizado por otro usuario en la base de datos (los usernames deben ser llaves únicas). Código de error: `USERNAME_TAKEN`.